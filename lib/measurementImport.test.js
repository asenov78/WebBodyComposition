...
describe('importMeasurementRecords', () => {
    it('imports a new, valid record', async () => {
        ...
        const record = { date: '2025-11-18T13:55:41.000Z', weightKg: 75.5, bmi: 23.8, bodyFat: 20.5 };
        const result = await importMeasurementRecords(USER_ID, [record]);

        expect(result).toEqual({ imported: 1, duplicates: 0, skipped: 0, total: 1, newRecords: [record] });
    ...
    it('counts an already-imported record as a duplicate, not a re-import', async () => {
        ...
        expect(result).toEqual({ imported: 0, duplicates: 1, skipped: 0, total: 1, newRecords: [] });
    ...
    it('skips a record with no date instead of importing it as "today"', async () => {
        ...
        expect(result).toEqual({ imported: 0, duplicates: 0, skipped: 1, total: 1, newRecords: [] });
        expect(mockCreate).not.toHaveBeenCalled();
    ...
    it('processes a mixed batch and tallies each outcome independently', async () => {
        ...
        const newRecord = { date: '2025-11-18T13:55:41.000Z', weightKg: 75.5 }; // new
        const result = await importMeasurementRecords(USER_ID, [
            newRecord,
        ...
        expect(result).toEqual({ imported: 1, duplicates: 1, skipped: 1, total: 3, newRecords: [newRecord] });
    });
